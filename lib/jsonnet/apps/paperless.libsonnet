...
function(params) {
  ...
  config: {
    ...
    data: $._config.config {
      ...
      PAPERLESS_CORS_ALLOWED_HOSTS: 'http://%(name)s-headless.%(namespace)s.svc,http://%(name)s.%(namespace)s.svc,https://%(domain)s' % $._config,
    },
  },
  ...
  local c = {
    ...
    env: [
      ...
      },
      {
        ...
        value: $.serviceHeadless.metadata.name + '.' + $._config.namespace + '.svc,' + $.service.metadata.name + '.' + $._config.namespace + '.svc,$(POD_IP)',
      },
      {
        name: 'PAPERLESS_PORT',
        value: '8000',
