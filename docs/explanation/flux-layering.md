...
Those values ConfigMaps keep stable names, which is what lets a `valuesFrom`
entry reference one directly. What makes a change to one reach its release is a
label rather than a new name. See
...
`--with-source` does both in one command, and is the shorter way to say it:

```bash
flux -n <namespace> reconcile kustomization <name> --with-source
```

There is no third step for a component whose values come from a
`configMapGenerator`. There used to be: the ConfigMap's contents changed while
its name did not, so the HelmRelease spec never moved and helm-controller had no
event. Those ConfigMaps now carry `reconcile.fluxcd.io/watch: Enabled` and
helm-controller reconciles on the write; see
[why Helm values live in a file](helm-values.md).

## What still reconciles green without your change in it

Two cases survive, and in neither will the status show anything wrong.
...
The same shape catches the objects Kyverno generates — a namespace's
`flux-reconciler`, its RoleBinding, its `GitRepository`. They are in no
Kustomization's inventory, so an edit to one is reverted rather than applied,
and `generate-flux-reconciler` recreates it from the Namespace's label. Edit the
