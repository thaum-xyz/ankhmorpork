...
`flux-reconciler` identity the Kustomization applies as
(`generate-flux-reconciler`) and the `GitRepository` it reads
(`generate-flux-source`): a Kustomization can create neither, and both are the
same for every app, so a file per namespace would only be a place for them to
drift. Three properties of the directory are kept on purpose. There is no
...
generated by `generate-flux-reconciler` and `generate-flux-source` from a label
on its Namespace. The difference is the bootstrap path: the Kustomization that
